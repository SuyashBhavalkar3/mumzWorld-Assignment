# ---------------------------------------------------------
# ARCHITECTURE: Persistent AI Ecosystem
# 1. SQLAlchemy Layer: Handles SQLite (Local) & Postgres (Production)
# 2. SHA-256 Hashing: Enables ultra-fast caching of image analyses.
# 3. SSE Streaming: Delivers real-time AI responses to the UI.
# ---------------------------------------------------------
...
async def analyze_product(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Hashing allows us to identify redundant queries without calling OpenAI.
        contents = await file.read()
        image_hash = hashlib.sha256(contents).hexdigest()
        
        # Step 1.5: Check Persistent Cache (SQLAlchemy Query)
        # If the product was analyzed before, we return the result in <10ms.
...
async def chat_with_expert(request: ChatRequest):
    ...
    try:
        ...
        def generate():
            # Create a streaming connection to OpenAI.
            # This yields tokens as they are generated by the model.
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=messages,
                max_tokens=300,
